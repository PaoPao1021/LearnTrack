import { db, getSetting, SETTINGS_KEYS } from '../db/database';
...
export async function saveEntryWithProgress(
  ...
  if (input.linkedPathId && input.quantityDelta) {
    ...
    if (path && path.mode === 'quantity') {
      const opId = uuid2();
      const newCompleted = Math.max(0, (path.completedQuantity ?? 0) + input.quantityDelta);
      ...
      await db.transaction('rw', db.entries, db.paths, db.progressEvents, db.pendingOps, async () => {
        ...
        const groupId = uuid2();
        await enqueueOp('entry', entry.id, entry, null, groupId);
        await enqueueOp('path', updated.id, updated, path.version, groupId);
        await enqueueOp('progressEvent', progress!.id, progress, null, groupId);
      });
...
export async function updateEntry(id: string, patch: Partial<EntryRecord>): Promise<void> {
  ...
  await enqueueOp('entry', id, updated, existing.version);
}
  ...
  if (undoProgress && existing.linkedPathId) {
    ...
    if (path) {
      ...
      if (totalDelta !== 0) {
        ...
        await enqueueOp('path', path.id, updatedPath, path.version);
        for (const ev of events) {
          const undone: ProgressEvent = { ...ev, undoneAt: nowIso() };
          await db.progressEvents.put(undone);
          await enqueueOp('progressEvent', ev.id, undone, null);
        }
      } else {
...
export async function addCategory(level: Category['level'], parentId: string | null, name: string, color: string): Promise<Category> {
  ...
  await enqueueOp('category', row.id, row, null);
  return row;
}
...
export async function archiveCategory(id: string): Promise<void> {
  ...
  await enqueueOp('category', id, updated, existing.version);
}
...
export async function createPath(input: {
  ...
  await enqueueOp('path', path.id, path, null);
  return path;
}
...
export async function addPathItems(pathId: string, titles: string[]): Promise<void> {
  ...
  const groupId = uuid2();
  for (const r of rows) await enqueueOp('pathItem', r.id, r, null, groupId);
}
...
export async function setItemDone(item: PathItem, done: boolean): Promise<void> {
  ...
  await enqueueOp('pathItem', item.id, updated, item.version);
  ...
  await enqueueOp('progressEvent', event.id, event, null);
}

export async function addQuantity(pathId: string, delta: number): Promise<void> {
  const path = await db.paths.get(pathId);
  if (!path) return;
  const next = Math.max(0, (path.completedQuantity ?? 0) + delta);
  ...
  const event: ProgressEvent = {
    ...
    quantityDelta: delta, itemDone: null, entryId: null, undoneAt: null, createdAt: nowIso(),
  };
  await db.progressEvents.add(event);
  await enqueueOp('path', pathId, updated, path.version);
  await enqueueOp('progressEvent', event.id, event, null);
}
...
export async function addTodo(input: { title: string; subjectId: string | null; scheduledDate: string | null; dueDate: string | null }): Promise<void> {
  ...
  await enqueueOp('todo', todo.id, todo, null);
}
...
export async function toggleTodo(todo: Todo): Promise<void> {
  ...
  await enqueueOp('todo', todo.id, updated, todo.version);
}
...
export async function deleteTodo(id: string): Promise<void> {
  ...
  await db.todos.put(updated);
  await enqueueOp('todo', id, updated, todo.version);
}
...
export async function setGoal(input: { scope: Goal['scope']; subjectId: string | null; period: Goal['period']; targetSeconds: number }): Promise<void> {
  ...
  if (existing) {
    ...
    await enqueueOp('goal', existing.id, updated, existing.version);
    return;
  }
