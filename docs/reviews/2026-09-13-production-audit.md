...
## 二轮审计（2026-09-14，图表优化后）

统计页图表重构（GitHub 风格热力图、扇形图中心总量、趋势渐变面积）后的增量审计：

| # | 发现 | 处置 |
|---|------|------|
| R2-1 | `echarts/core` 按需注册缺少 `TitleComponent`，扇形图中心总量不渲染（静默失败） | ✅ 已注册并在浏览器验证 |
| R2-2 | 具体科目排行图 x 轴 `splitLine` 未关闭，图中出现游离竖线 | ✅ 已关闭 |
| R2-3 | `useChart` 对初始化异常无兜底，单图配置错误会中断整个页面渲染 | ✅ try/catch + console.error，单图失败不再影响页面 |
| R2-4 | 趋势渐变直接拼接 `color + '3d'`，非 6 位 hex 的自定义色会画出非法色值 | ✅ 加 hex 格式守卫，非法色回退中性灰 |
| R2-5 | 热力图年份在跨午夜后不会刷新（useMemo 空依赖） | ✅ 以 `today` 作为依赖 |
| R2-6 | 疑似缺陷：小时分布有柱但覆盖率显示 0% —— 核查 domain 后确认口径一致（覆盖率=有起止时间时长占比，计时 3 秒/总 30 分钟被四舍五入为 0%），非缺陷，记录在案 | 无需修复 |

复审结果：`npm run build`、21 项测试、`npm audit` 0 漏洞通过；canvas 像素采样确认扇形图中心文字与热力图色阶实际绘制；热力图格子点击下钻实测正常（返回当日记录）。
