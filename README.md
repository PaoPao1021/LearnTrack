...
仓库附带的 Compose / Nginx 配置默认只监听 HTTP 80，因此默认使用非 Secure 会话 Cookie。公网部署时应先配置 TLS，再将 `LT_COOKIE_SECURE` 设为 `true`。
...
- 自动化测试 21 项通过：领域规则 14 项、API 认证/同步 2 项、Web 队列/备份/命令 5 项。
- 全量 TypeScript + Vite 构建通过，`npm audit` 为 0 个已知漏洞。
- 浏览器实测：桌面端与移动端总览、按需加载的统计页，控制台 0 错误 / 0 警告。

未实测（如实标注）：Docker Compose 实际启动（当前审查环境未安装 Docker）、真实手机锁屏 / PWA 安装实机、双设备并发冲突 UI 全流程、5 万条性能压测。
